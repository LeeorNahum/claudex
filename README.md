...
The `/model` picker shows a `background-summaries` entry labeled "Custom Haiku model". That is not a model you pick: it's where claudex points Claude Code's internal background calls (session summaries for `--resume` and similar), a proxy alias of the family's lightest tier named so the row explains itself. Claude Code offers no way to hide a tier override from the picker.

**Never run `cli-proxy-api -claude-login`.** It's tempting if something requests a real Anthropic model and gets a `502 unknown provider` from the proxy, since that command looks like the obvious fix. It is not: it routes your real Claude subscription's OAuth token through a third-party tool, which violates Anthropic's Consumer Terms. Anthropic has been enforcing this without warning since early 2026, with real accounts suspended within minutes. claudex pins subagents to the session's own model specifically so no real work ever asks the proxy for an Anthropic model. Native Claude models are what plain `claude` is for.
